...
import {StyleSheet,View,Text,SafeAreaView,Image,TextInput,TouchableOpacity,Alert,AsyncStorage} from "react-native";
      ...
      <SafeAreaView style={body}>
        ...
        {this.state.loginType == "注册" ? (
          <Text style={styles.changeType}>
            已有账号？请<Text style={{color:'blue'}} onPress={() => this.changeType()}>登录</Text>
          </Text>
        ) : null}
        {this.state.loginType == "登录" ? (
          <Text style={styles.changeType}>
            没有账号？请<Text style={{color:'blue'}} onPress={() => this.changeType()}>注册</Text>
          </Text>
        ) : (
          <View />
        )}
